...
type Authorizer = (token: TokenSet, userInfo: UserinfoResponse) => Promise<void>;
...
class Proxy {
    ...
    constructor(private client: Client, opts: ProxyOpts = {}) {
        ...
        this.authorize = opts.authorizer || async function() {};
    }
     ...
     * @return {*}
     */
    private handleLogin(request: CloudFrontRequest, currentUser: null|JWTPayload) {
        ...
        const redirect_uri = `${this.getBaseUrl(request)}${this.pathCallback}?destination=${next}`;
        ...
        const response = {
            ...
            headers: {
                ...
                "set-cookie": [
                    {key: 'Set-Cookie', value: cookie.serialize(this.getAuthCookieName('state'), state, {httpOnly: true, secure: true, path:  '/'})},
                ]
            }
        }
     ...
     */
    private async handleCallback(request: CloudFrontRequest, currentUser: null|JWTPayload) {
        const qs = new URLSearchParams(request.querystring);
        const state = qs.get('state') ?? undefined;
        const code = qs.get('code') ?? undefined;
        const next = this.filterDestination(qs.get('destination'));
        const expected_state = cookie.parse(request.headers.cookie?.[0].value ?? '')[this.getAuthCookieName('state')];

        const redirect_uri = `${this.getBaseUrl(request)}${this.pathCallback}?destination=${next}`;

        try {
            const tokenSet = await this.client.oauthCallback(redirect_uri, {code, state}, {
                response_type: 'code',
            ...
            return this.sendTo(next, request, currentUser, {
                "set-cookie": [
                    {key: 'Set-Cookie', value: cookie.serialize(this.getAuthCookieName(), token, {httpOnly: true, secure: true, path:  '/'})},
                    {key: 'Set-Cookie', value: cookie.serialize(this.getAuthCookieName('state'), '', {httpOnly: true, secure: true, path:  '/', expires: new Date(0)})},
                ]
            })
        }
        catch(err) {
     ...
     * @return {Promise<Response>}
     */
    private handleLogout(request: CloudFrontRequest, currentUser: null|JWTPayload) {
        const qs = new URLSearchParams(request.querystring);
        const next = this.filterDestination(qs.get('destination'));
        return this.sendTo(next, request, currentUser, {
            "set-cookie": [{key: 'Set-Cookie', value: cookie.serialize(this.authCookieName, '', {httpOnly: true, secure: true, path: '/', expires: new Date(0)})}]
        })
    }
     ...
     * @return {*}
     */
    private handleRestricted(request: CloudFrontRequest, currentUser: null|JWTPayload) {
        if(currentUser !== null) {
     ...
     */
    private sendTo(destination: string, request: CloudFrontRequest, currentUser: null|JWTPayload, headers = {}) {
        return Promise.resolve({
            status: '302',
     ...
     * @return {*}
     */
    private async getCurrentUser(request: CloudFrontRequest): Promise<null|JWTPayload> {
